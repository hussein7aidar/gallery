...
class Album {
  ...
  /// Folder names / relative-path fragments that indicate an album was created
  /// automatically by another app or the system (rather than by the user).
  // NOTE: 'screenshot' is intentionally excluded — Screenshots is shown as a
  // normal top-level album, not grouped under "Others".
  static const List<String> autoMarkers = [
    'whatsapp', 'telegram', 'instagram', 'messenger', 'facebook', 'snapchat',
    'twitter', 'wechat', 'viber', 'signal', 'tiktok', 'discord', 'line',
    'pinterest', 'reddit', 'screen record', 'screen-record',
    'download', 'bluetooth', 'gif', 'gboard', 'sticker', 'documents',
  ];

  /// Whether this album was auto-generated by another app or the system, used
  /// to group it under "Others". User-created and renamed-by-the-user albums,
  /// and the locked "All Photos", are never auto-generated.
  bool get isAutoGenerated {
    if (isLocked || customization.userCreated) return false;
    final name = originalName.toLowerCase();
    final rp = (coverAsset?.relativePath ?? '').toLowerCase();
    // Anything living under an app's scoped media directory is app-generated.
    if (rp.contains('android/media/')) return true;
    return autoMarkers.any((m) => name.contains(m) || rp.contains(m));
  }
